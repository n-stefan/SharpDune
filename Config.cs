...
namespace SharpDune
{
	...
	class Config
	{
		...
		internal static bool Config_Read(string filename, DuneCfg config)
		{
			...
			config.useMouse = Convert.ToBoolean(buffer[4]); //BitConverter.ToBoolean(buffer[4..5]);
			config.useXMS = Convert.ToBoolean(buffer[5]); //BitConverter.ToBoolean(buffer[5..6]);
